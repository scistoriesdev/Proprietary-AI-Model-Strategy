...
- **Additional Navigation Text Fixes** - Standardized remaining link text across all pages:
  - index.html card titles
  - training.html footer nav and inline links
  - hardware.html footer nav and inline links  
  - legal.html footer nav
...
- **Standardized Navigation Links** - All pages now use single-word nav links for consistency:
  - "Scientific Foundation" → "Scientific"
  - "Cloud Strategy" → "Cloud"
  - Updated across: index.html, training.html, scientific.html, hardware.html, cloud.html, legal.html, readme.html
...
- **Search on README Page** - Added missing `app.js` script include to `readme.html`, enabling search functionality
...
- **Legal Page Tab Navigation** - Fixed inconsistent tab switching behavior
  - Updated `switchLegalTab()` function to handle all 10 tabs (was only handling 5)
  - Fixed styling toggle for active/inactive tab states
  - All tabs now properly show/hide content: Ownership, Training Data, Model Licenses, Contracts, Case Law, Jurisdiction, Commercial, Medical, Ethics, Insurance
...
- **Navigation Title** - Updated to "SciStories Proprietary Model Strategy" across all pages
